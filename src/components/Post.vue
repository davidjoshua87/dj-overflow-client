    ...
    <div>
        ...
        <div class="body-post">
            <div class="post-title">
                <p>
                    <span v-html="showThread.title"></span>
                </p>
            </div>
            <div class="post-desc">
                <div class="q-vote">
                ...
                <span class="q-desc" v-html="showThread.description"></span>
            </div>
            ...
            <div class="post-answer" v-for="(ans, i) in showThread.answer" :key='i'>
                ...
                <div class="delbutton">
                    <button v-if='ans.user === user' @click='remove(ans._id)' class="rad-button danger flat"
                        id="rad-button">delete</button>
                ...
                <div class="formsection">
                    <div class="commentform">
                        <editor api-key="e35okpdgs5wel4anlmpc3mambugjgzs7bye50qeitpamkhgj" v-validate="'required'"
                            name="comment" id="comment" v-model='comment' placeholder="Enter your comment here..">
                        </editor>
                        <br>
                        <span style="color:red;font-size:10px;" v-show="errors.has('comment')"
                            class="help is-danger">{{ errors.first('comment') }}</span>
                    </div>
                    <div class="commentbutton">
                        <button @click='submitAnswer' class="rad-button good flat"
                            style="float:right; margin-right:2em">Post Answer</button>
                    </div>
                    <div class="commentbutton">
                        <button @click='back' class="rad-button good flat"
                            style="float:right; margin-right:2em">Cancel</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                this.$http.get(`/questions/${quesId}/upvote/${this.user}`)
                    ...
                    .catch(err => {
                        swal({
                            type: 'error',
                            title: 'Oops...',
                            text: 'user cannot vote their own question'
                        })
                    })
            },
