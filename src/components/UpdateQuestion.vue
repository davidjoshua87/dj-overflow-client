<template>
    <div class="modal" :id="'updatequestion' + index">
        <div class="form">
            <h2>Update Question</h2><span style="float:right;cursor:pointer;margin-top:-3em;"
                @click='closemodal(index)'>&times;</span>
            <p type="Title">
                <input v-validate="'required'" name="update-title" :id="'update-title' + index" :value='question.title'>
                <span style="color:red;font-size:10px;" v-show="errors.has('update-title')"
                    class="help is-danger">{{ errors.first('update-title') }}</span>
            </p>
            <p type="Description">
                <editor api-key="e35okpdgs5wel4anlmpc3mambugjgzs7bye50qeitpamkhgj" v-validate="'required'"
                    name="update-description" :id="'update-description' + index" :value='question.description'></editor>
                <span style="color:red;font-size:10px;" v-show="errors.has('update-description')"
                    class="help is-danger">{{ errors.first('update-description') }}</span>
            </p>
            <div class="buttons">
                <button @click='closemodal(index)' class="rad-button warning flat" id="rad-button">Cancel</button>
                <button @click='updatequestion(question._id, index)' class="rad-button good flat"
                    id="rad-button">Update</button>
            </div>
        </div>
        {{question}}
    </div>
